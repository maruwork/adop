...
def _run_to_open_trial(
    run, root, *, scene: str = "ci-format", tool: str = "ruff", known_attrs: bool = False
) -> str:
...
def test_close_trial_double_close_is_rejected(run, root):
    ...
    assert (
        run(
            ...
            "great",
            "--judgment-reason",
            "trial succeeded for the bounded scene",
            "--next-action",
            "promote the formatter into CI",
            "--recurring-control-decision",
            "yes",
            "--root-cause-hypothesis",
            "the project needed a stable formatter path",
            "--preventive-action",
            "document the approved formatter usage scene",
            "--why-this-problem-recurred",
            "the team had no prior explicit formatter adoption record",
        )
        == 0
    )
...
def test_reject_from_proposed_resolves_reject(run, root):
    ...
    assert (
        run("reject", "--artifact-root", root, "--use-case", "r", "--reject-reason", "not worth it")
        == 0
    )
...
def test_reject_requires_history(run, root):
    assert (
        run("reject", "--artifact-root", root, "--use-case", "empty", "--reject-reason", "no") == 5
    )
