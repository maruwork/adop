...
| `tests/test_usability_commands.py` | repo maintainer | isolated positive and negative proof shelf for `actionlint`, `eslint`, `hadolint`, `markdownlint-cli2`, `prettier`, `renovate`, `shellcheck`, `trivy`, and `vscode-eslint`; not an operational landing target |
