...
class AdopBoundaryError(AdopArtifactError):
    """Artifact-root boundary violation (distinct exit code from plain IO errors)."""
...
def ensure_artifact_root(
    ...
    *,
    ...
    try:
        ...
        # A JSON-native CLI must not leak a raw traceback when the root cannot be
        # created (permission denied, path-under-a-file, bad drive). Surface it as
        # an artifact IO error so the CLI wraps it in a JSON error response.
        raise AdopArtifactError(f"cannot create artifact root '{resolved}': {exc}") from exc
    return resolved
